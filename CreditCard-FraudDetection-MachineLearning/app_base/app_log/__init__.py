...
def _format_msg_qiyu(name, content):
    return 'function:[%s] $$error: [%s]' % (str(name), str(content))


def error_qiyu(name, content):
    logging.getLogger('_log').error(_format_msg_qiyu(name, content))
